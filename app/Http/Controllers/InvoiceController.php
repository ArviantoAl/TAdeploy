...
//            }
    ...
    public function teknisi_kirim_balik(Request $request){
        $bulans=Carbon::now()->format('n');
        $tahun=Carbon::now()->format('Y');
        $bank = Bank::all();
        $tgl_tempo=$request->tempo;
...
//            }
    ...
    public function keuangan_kirim_balik(Request $request){
        $bulans=Carbon::now()->format('n');
        $tahun=Carbon::now()->format('Y');
        $bank = Bank::all();
        $tgl_tempo=$request->tempo;
...
//            }
    ...
    public function setujui_pembayaran($id_invoice)
    {
        ...
        return redirect()->route('teknisi.invoice')->with('success', 'Invoice '.$id_invoice.' telah disetujui!');
    }

    public function keuangan_setujui_pembayaran($id_invoice)
    {
        $invoices = Invoices::query()->find($id_invoice);
        $id_pelanggan = $invoices->pelanggan_id;
        $user = User::query()->find($id_pelanggan);
        ...
        return redirect()->route('keuangan.invoice')->with('success', 'Invoice '.$id_invoice.' telah disetujui!');
    }
    ...
    public function keuangan_setujui_manual(Request $request, $id_invoice)
    {
        $metode = $request->metode;
        $bank = $request->bank;
        if ($metode==0){
    ...
    public function keuangan_tolak_pembayaran($id_invoice)
    {
        $invoices = Invoices::query()->find($id_invoice);
        $id_pelanggan = $invoices->pelanggan_id;
        ...
        return redirect()->route('keuangan.invoice')->with('success', 'Invoice '.$id_invoice.' tidak dibayar!');
    }
